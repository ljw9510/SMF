...
class SNMF():
    ...
    # 1) NMF + Logistic Regression
    # 2) NMF + Linear Regression
    ...
    def update_dict_joint_logistic_subsample(self, X, H, W0,
                                   r, a1=0, a2=0, sub_iter=2,
                                   stopping_diff=0.1,
                                   nonnegativity=True,
                                   subsample_size=None):
        '''
        ...
        while (i < sub_iter) and (dist > stopping_diff):
            ...
            for k in np.arange(W1.shape[1]): #
                grad_MF = (W1 @ H - X[0]) @ H[k,:].T
                # grad_pred = X[0] @ (P-X[1]).T @ W0[1][:, 1:] # exclude the first column of W[1] (constant terms)
                grad_pred = X[0] @ (P-X[1]).T @ W0[1][:, k+1] # exclude the first column of W[1] (constant terms)
                grad = self.xi * grad_MF + grad_pred + a1 * np.sign(W1[:,k])*np.ones(shape=[len(idx),]) + a2 * W1[:,k]
                # grad = grad_MF

                W1[:,k] -= (1 / (((i + 10) ** (0.5)) * (A[k,k] + 1))) * grad

                if nonnegativity:
                    W1[:,k] = np.maximum(W1[:,k], np.zeros(shape=[len(idx),]))  # nonnegativity constraint

                if r is not None:  # usual sparse coding without radius restriction
                    d = np.linalg.norm(W1[idx,:] - W0[0][idx,:], 2)
                    W1[idx,k] = W0[0][idx,k] + (r / max(r, d)) * (W1[idx,k] - W0[0][idx,k])
                W0[0] = W1
    ...
    def train_logistic(self,
        ...
        for step in trange(int(iter)):
            ...
            if self.full_dim:
                ...
                X0_comp = W[0].T @ X[0]
                # X0_comp = (X0_comp + H)/2
                # print('X0_comp mean', np.mean(X0_comp))
                #print('X0_comp std', np.std(X0_comp))
                # X0_comp -= np.mean(X0_comp)
                # X0_comp /= np.linalg.norm(X0_comp)
                # X0_comp /= np.max(X0_comp)

                """
                W[1] = self.update_beta_logistic(X[1], W[1], X0_comp, r=search_radius,
                                                 a1=self.L1_reg[2],
            ...
            if (step % 10) == 0:
                ...
                #if not self.full_dim:
                #    print('!!! W norm', np.linalg.norm(W[0].reshape(-1,1)))
    ...
    def train_benchmark(self,
        ...
        if self.full_dim: # logistic regression without dimension reduction
            ...
            for step in trange(int(iter)):
                ...
                H = update_code_within_radius(X[0], W[0], H0=self.ini_code, r=search_radius, a1=self.L1_reg[0],
                                               sub_iter=[2],
        ...
        if if_compute_recons_error:
            # print the error every 50 iterations
            if self.full_dim:
                error_data = np.linalg.norm((X[0] - H).reshape(-1, 1), ord=2)
            else:
                error_data = np.linalg.norm((X[0] - W[0] @ H).reshape(-1, 1), ord=2)
        ...
        if if_validate and (step>1):
            self.validation(result_dict = result_dict, verbose=True)
            threshold = result_dict.get('Opt_threshold')
            AUC = result_dict.get('AUC')


        self.result_dict = self.validation(result_dict = result_dict)
        #threshold = result_dict.get('Opt_threshold')
        #AUC = result_dict.get('AUC')
        #print('!!! FINAL [threshold, AUC] = ', [np.round(threshold,3), np.round(AUC,3)])

        print('!!! saved validation AUC:', self.result_dict.get('AUC'))
        return result_dict
    ...
    def validation(self,
        ...
        Y_pred = P_pred.copy()
        Y_pred[Y_pred < mythre] = 0
        Y_pred[Y_pred >= mythre] = 1
        ...
        error_data = np.linalg.norm((X_test[0] - W[0] @ H_test).reshape(-1, 1), ord=2)**2
        rel_error_data = error_data / np.linalg.norm(X_test[0].reshape(-1, 1), ord=2)**2
        ...
        """
        # block prediction
        P_pred_new, H_new = self.predict(X_test = self.X_test[0], W=self.loading)
        fpr, tpr, thresholds = metrics.roc_curve(self.X_test[1][0, :], P_pred_new[0,:], pos_label=None)
        mythre = thresholds[np.argmax(tpr - fpr)]
        myauc_new = metrics.auc(fpr, tpr)
        # if verbose:
        #     print('!!! AUC_block', myauc_new)
        result_dict.update({'AUC_block': myauc_new})
        """
    ...
    def predict(self,
        ...
        for step in range(int(iter)):
            ...
            # Update code
            H = self.update_code_joint_logistic(X, W, H, r=None)
